...
def native_name_from_type(ntype, underlying=False):
    ...
    if not underlying and kind == cindex.TypeKind.ENUM:
        ...
        if decl.spelling == "string" and parent and parent.spelling == "std":
            ...
            # print("probably a function pointer: " + str(decl.spelling))
            return const + decl.spelling
    else:
        # name = ntype.get_declaration().spelling
        # print("Unknown type: " + str(kind) + " " + str(name))
...
class NativeType(object):
    ...
    def from_native(self, convert_opts):
        ...
        return "native_to_luaval(tolua_S, ret)"
    ...
    def to_native(self, convert_opts):
        ...
        arg_name = convert_opts['out_value']
        func_name = convert_opts['func_name']
        level = convert_opts['level']
        arg_idx = convert_opts['arg_idx']
        return "ok &= luaval_to_native(tolua_S, "+str(arg_idx)+", &"+arg_name+", \""+func_name+"\")"
...
class NativeField(object):
    def __init__(self, cursor, parent_class=None):
        ...
        # self.class_name = parent_class.class_name if parent_class else None
    ...
    def generate_code(self, current_class = None, generator = None):
        ...
        if self.ntype.is_function:
            tpl_file = 'public_field_lambda.c'
        else:
            tpl_file = 'public_field.c'
        tpl = Template(file=os.path.join(gen.target, "templates", tpl_file),
                       searchList=[current_class, self])
...
class NativeOverloadedFunction(object):
    def __init__(self, func_array):
        ...
        self.reg_name = func_array[0].reg_name
...
class NativeClass(object):
    def __init__(self, cursor, generator):
        ...
        self.parse()
        print('class: ' + self.namespaced_class_name)
    ...
    def generate_code(self):
        ...
        if self.generator.script_type == "lua":
            docfuncfilepath = os.path.join(self.generator.outdir + DOC_DIR_POSTFIX, self.class_name + ".lua")
        ...
        self.doc_func_file.write(str(apidoc_classhead_script))
        '''
        ...
        methods = []
        for m in self.methods_clean():
            methods.append({"name": m["name"], "impl": m['impl'], "override": False})
        for m in self.static_methods_clean():
            methods.append({"name": m["name"], "impl": m['impl'], "override": False})
        if self.generator.script_type == "lua":
            for m in self.override_methods_clean():
                methods.append({"name": m["name"], "impl": m['impl'], "override": True})
        ...
        self.doc_func_file.write(str(apidoc_classfoot_script))
        if self.generator.script_type == "lua":
...
class NativeEnum(object):
    def __init__(self, cursor, generator, inclass=None):
        # the cursor to the implementation
        self.cursor = cursor
        self.class_name = cursor.displayname
        self.namespaced_class_name = self.class_name
        ...
        self.ns = None
        if inclass:
            self.parents.append(inclass)
        self.comment = _get_comment(cursor.raw_comment)
        #for generate lua api doc
        ...
        self.namespace_name        = get_namespace_name(cursor)
        self.parse()
        print('enum: ' + self.namespaced_class_name)
    ...
    def _process_node(self, node):
        if node.kind == cindex.CursorKind.ENUM_CONSTANT_DECL:
            ...
            field["comment"] = _get_comment(node.raw_comment)
    ...
    def generate_code(self):
        ...
        # no doc file when nested
        if len(self.parents) > 0:
            return

        docenumfilepath = os.path.join(
            self.generator.outdir + DOC_DIR_POSTFIX, self.class_name + ".lua")
        self.doc_enum_file = open(docenumfilepath, "w+")
        apidoc_enum_script  = Template(file=os.path.join(self.generator.target,
                                                         "templates",
                                                         "apidoc_enum.script"),
                                       searchList=[{"current_class": self, "generator": self.generator}])
        self.doc_enum_file.write(str(apidoc_enum_script))
        self.doc_enum_file.close()
...
class Generator(object):
    def __init__(self, opts):
        ...
        self.function_lower_initial = opts['function_lower_initial']
        self.use_dot_sfunction = opts['use_dot_sfunction']
        ...
        if opts['rename_classes']:
            ...
            for rename in list_of_class_renames:
                ...
                # print 'rename class [' + class_name + '] to [' + renamed_class_name + ']'
    ...
    def should_rename_function(self, class_name, method_name):
        ...
        if class_name in self.rename_functions and method_name in self.rename_functions[class_name]:
            new_name = self.rename_functions[class_name][method_name]
        elif self.function_lower_initial:
            new_name = method_name[0].lower() + method_name[1:]
        if new_name:
            print('rename function %s::[%s] -> [%s]' % (class_name, method_name, new_name))
        return new_name

    def get_class_or_rename_class(self, class_name):
        if class_name in self.rename_classes:
            new_name = self.rename_classes[class_name]
            print('rename class [%s] -> [%s]' % (class_name, new_name))
            return new_name
        return class_name
    ...
    def generate_code(self):
        ...
        docfiledir   = self.outdir + DOC_DIR_POSTFIX
    ...
    def _deep_iterate(self, cursor, depth=0, inclass=None):
        ...
        if cursor.kind == cindex.CursorKind.CLASS_DECL:
                    ...
                    if not cursor.displayname in self.generated_classes:
                        ...
                        for node in cursor.get_children():
                            if node.kind == cindex.CursorKind.ENUM_DECL:
                                nested_enums.append(NativeEnum(node, self, inclass=inclass))
            ...
            if cursor == cursor.type.get_declaration() and len(get_children_array_from_iter(cursor.get_children())) > 0:
                ...
                if is_targeted_class and len(cursor.displayname) > 0 and self.in_listed_classes_exactly(cursor.displayname) and not inclass:
                    if not cursor.displayname in self.generated_classes:
                        nclass = NativeEnum(cursor, self, inclass=inclass)
                        nclass.generate_code()
                        self.generated_classes[cursor.displayname] = nclass
                    return
        ...
        for node in cursor.get_children():
            # print("%s %s - %s" % (">" * depth, node.displayname, node.kind))
            self._deep_iterate(node, depth + 1, inclass)
    ...
    def scriptname_from_native(self, namespace_class_name, namespace_name):
        ...
        if namespace_class_name.find("::") >= 0:
            if namespace_class_name.find("std::") == 0:
                ...
                namespace_name = namespace_class_name[:namespace_class_name.find("::")] + '::'
                for (k, v) in script_ns_dict.items():
                    if k == namespace_name:
                        return namespace_class_name.replace("*","").replace("const ", "").replace(k, v)
    ...
    def lua_typename_from_natve(self, namespace_class_name, is_ret = False, nt = None):
        script_ns_dict = self.config['conversions']['ns_map']
        ...
        for ty in map_type:
            if ncn.startswith(ty):
                ...
                if len(e) == 2:
                    ...
                    if len(e0) > 0 and len(e1) > 0 and e0 in LUA_TYPE_MAP and e1 in LUA_TYPE_MAP:
        ...
        if ncn.startswith("std::function<"):
            if nt is not None and nt.is_function:
                ...
                if str(nt.ret_type) != "void":
                    ret_str = self.lua_typename_from_natve(nt.ret_type.namespaced_name)
                    ret_str = ":" + ret_str
                for i in range(len(nt.param_types)):
                    param = nt.param_types[i]
                    param_str = self.lua_typename_from_natve(param.namespaced_name)
        ...
        for (k, v) in script_ns_dict.items():
            if k in ncn:
                '''
                if ncn.find("cocos2d::Vec2") == 0:
                    return "vec2_table"
                if ncn.find("cocos2d::Vec3") == 0:
                    return "vec3_table"
                if ncn.find("cocos2d::Vec4") == 0:
                    return "vec4_table"
                if ncn.find("cocos2d::Vector") == 0:
                    for k, v in LUA_TYPE_MAP.items():
                        if ncn.find(k) == len("cocos2d::Vector"):
                            return v + "[]"
                    return "array_table"
                if ncn.find("cocos2d::Mat4") == 0:
                    return "mat4_table"
                if ncn.find("cocos2d::Map") == 0:
                    return "map_table"
                if ncn.find("cocos2d::Point")  == 0:
                    return "point_table"
                if ncn.find("cocos2d::Size")  == 0:
                    return "size_table"
                if ncn.find("cocos2d::Rect")  == 0:
                    return "rect_table"
                if ncn.find("cocos2d::Color3B") == 0:
                    return "color3b_table"
                if ncn.find("cocos2d::Color4B") == 0:
                    return "color4b_table"
                if ncn.find("cocos2d::Color4F") == 0:
...
def main():
    ...
    for t in targets:
        ...
        print("  Generating bindings for target", t)
        for s in sections:
            print("    Processing section", s, "\n")
            gen_opts = {
                'prefix': config.get(s, 'prefix'),
                'headers': (config.get(s, 'headers', raw=0, vars=dict(userconfig.items('DEFAULT')))),
                ...
                'clang_args': (config.get(s, 'extra_arguments', raw=0, vars=dict(userconfig.items('DEFAULT'))) or "").split(" "),
                ...
                'hpp_headers': config.get(s, 'hpp_headers', raw=0, vars=dict(userconfig.items('DEFAULT'))).split(' ') if config.has_option(s, 'hpp_headers') else None,
                'cpp_headers': config.get(s, 'cpp_headers', raw=0, vars=dict(userconfig.items('DEFAULT'))).split(' ') if config.has_option(s, 'cpp_headers') else None,
                'win32_clang_flags': (config.get(s, 'win32_clang_flags', raw=0, vars=dict(userconfig.items('DEFAULT'))) or "").split(" ") if config.has_option(s, 'win32_clang_flags') else None,
                'convert_legacy': config.get(s, 'convert_legacy') if config.has_option(s, 'convert_legacy') else None,
                'function_lower_initial': config.get(s, 'function_lower_initial') if config.has_option(s, 'function_lower_initial') else None,
                'use_dot_sfunction': config.get(s, 'use_dot_sfunction') if config.has_option(s, 'use_dot_sfunction') else None,
                }
